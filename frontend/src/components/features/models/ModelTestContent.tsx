...
interface ModelTestContentProps {
  modelId: string;
  modelName: string;
  providerName: string;
...
export function ModelTestContent({
  modelId,
  modelName,
  providerName,
  providerRouterUrl
}: ModelTestContentProps) {
  ...
  const handleTest = async () => {
    ...
    try {
      // Send a test message with the specific model
...
# Test ${modelName} model on ${providerName}
...
console.log(completion.choices[0].message.content);`;
  ...
  const curlExample = `# Test ${modelName} model on ${providerName}
    ...
    <div className="demo-container" style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      <div className="demo-header" style={{ flexShrink: 0 }}>
        ...
        <div className="flex items-center gap-2">
          ...
          <TooltipProvider>
            <Tooltip content={loading ? "Testing model..." : "Test model with active provider"}>
              <Button
                onClick={handleTest}
                disabled={loading}
                size="icon"
